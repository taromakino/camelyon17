...
def subsample(rng, df, n_subsample):
    if len(df) < n_subsample:
        return df
    else:
        idxs = rng.choice(len(df), n_subsample, replace=False)
        return df.iloc[idxs]


def make_data(batch_size, n_workers, n_subsample):
    ...
    if n_subsample is not None:
        df_train = subsample(rng, df_train, n_subsample)
        df_val_id = subsample(rng, df_val_id, n_subsample)
        df_val_ood = subsample(rng, df_val_ood, n_subsample)
        df_test = subsample(rng, df_test, n_subsample)
